...
bool gomoku_CheckDirection(const std::vector<Chess>& chesses, const Chess& lastChess, int dx, int dy, int countNeeded) {
...
// 检查三三禁手（黑棋不能同时形成两个活三）
bool gomoku_CheckDoubleThree(const std::vector<Chess>& chesses, const Chess& lastChess) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        // 检查是否形成活三（即可以发展成活四的三）
        if (gomoku_CheckDirection(chesses, lastChess, dx, dy, 3)) {
            // 检查两端是否为空（确保是活三）
...
// 检查四四禁手（黑棋不能同时形成两个四）
bool gomoku_CheckDoubleFour(const std::vector<Chess>& chesses, const Chess& lastChess) {
    if (lastChess.color != BLACK) return false; // 只有黑棋有禁手
    ...
    for (int i = 0; i < 4; i++) {
        ...
        // 检查是否形成四（活四或冲四）
        if (gomoku_CheckDirection(chesses, lastChess, dx, dy, 4)) {
            fours++;
...
// 检查黑棋是否触犯禁手
bool gomoku_CheckForbidden(const std::vector<Chess>& chesses, const Chess& lastChess) {
    if (lastChess.color != BLACK) return false; // 只有黑棋有禁手

    return gomoku_CheckDoubleThree(chesses, lastChess) || gomoku_CheckDoubleFour(chesses, lastChess);
}

// 检查是否获胜（修改后的版本，包含禁手判断）
int gomoku_CheckWin(const std::vector<Chess>& chesses, const Chess& lastChess) {
    if (lastChess.color == BLACK && gomoku_CheckForbidden(chesses, lastChess)) {
        return BLACK_FORBIDDEN; // 黑棋禁手
    }

    if (gomoku_CheckDirection(chesses, lastChess, 1, 0, 5) || // 水平
        gomoku_CheckDirection(chesses, lastChess, 0, 1, 5) || // 垂直
        gomoku_CheckDirection(chesses, lastChess, 1, 1, 5) || // 主对角线
        gomoku_CheckDirection(chesses, lastChess, 1, -1, 5)) { // 副对角线
