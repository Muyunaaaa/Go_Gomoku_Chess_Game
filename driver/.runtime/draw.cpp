...
void draw_Chess(const std::vector<Chess>& chesses) {
    std::cout << "[draw_Chess] chesses size = " << chesses.size() << std::endl;
    for (const auto& newchess : chesses) {
        std::cout << "[draw_Chess] Drawing at: (" << newchess.c_x << ", " << newchess.c_y << "), color = " << newchess.color << std::endl;
